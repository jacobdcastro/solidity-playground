...
contract Exchange is ERC20 {
    ...
    constructor(address _CryptoDevToken) ERC20("CryptDev LP Token", "CDLP") {
        require(
    ...
    function addLiquidity(uint _amountToProvide) public payable returns (uint) {
        uint liquidity;
        uint ethBalance = address(this).balance; // ETH Reserve
        uint cryptoDevTokenReserve = getReserve(); // CD Reserve
        ...
        if (cryptoDevTokenReserve == 0) {
            ...
            uint requiredCryptoDevTokenAmount = (msg.value *
                cryptoDevTokenReserve) / (ethReserve);
            require(
                _amountToProvide >= requiredCryptoDevTokenAmount,
    ...
    function removeLiquidity(uint _lpTokenAmount) public returns (uint, uint) {
        require(_lpTokenAmount > 0, "Amount should be greater than 0");
        ...
        uint ethAmount = (ethReserve * _lpTokenAmount) / _totalSupply;
        ...
        uint cryptoDevTokenAmount = (getReserve() * _lpTokenAmount) /
            _totalSupply;
